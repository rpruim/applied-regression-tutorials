...
1. Choose, and use R to create, an appropriate graphical display for a specified (combination of) variables. 

2. Plan and critique statistical graphics based on design principles.

:::{.callout-note}
## You NEED NOT memorize all the information in this section.
...
:::{.callout-tip}
### Treat this section like a reference manual, not a textbook

It's more important that you know how to look stuff up here than that you spend lots of time on ***every*** topic, and you don't need to memorize. Complete the first 4 sections if you are new to ggformula, and then refer to the others as needed and as time allows. Get used to using this site as a "manual" to look stuff up on demand.
:::

:::{.callout-note}
### Options/advanced sections

Graph types marked with an (o) and with their section header in
*italics* are ones that are optional and/or more advanced -- skip if you wish.
:::
...
-   Always put a space after a comment character (`#`).
...
Similar to a jitter plot is a sina plot -- the difference is that the sina plot shapes the dot cloud to indicate data density, rather than fitting all the dots into a rectangle.

We need the additional package `ggforce` to make a sina plot using the `gf_sina()` function.

Check it out...and play a bit! 

:::{#exr-sina}
What does the sina plot look like overlaid on boxplots? What if you adjust `size`, transparency `alpha`, or even `color`?
...
gf_boxplot(
  fct_reorder(faculty_type, percentage) ~ percentage, 
  .fun = mean,
  data = university_teachers)
```
...
Sometimes you might want to remove the legend title, or replace it with a more readable one. 
We do this by adjusting the associated **scale**.
We change scales inside `gf_refine()`.

:::{#exr-custom-legend-title}
Try the plot below with and without the `gf_refine()` line to see how it changes. Replace `"Activity"` with `""` (empty quotes) to remove the legend title entirely.
...
gf_bar(~AgeDecade, fill = ~PhysActive, 
       position = 'stack', 
       data = NHANES) |>
  gf_refine(scale_fill_discrete("Activity"))
```
:::
